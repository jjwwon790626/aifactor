...
# H5数据读取函数
def load_h5_data(feature_h5_path, label_h5_path, factor_name_path=None):
    """从H5文件读取特征和标签数据"""
    ...
    feature_matrix = features_df.astype(np.float64).values
    labels = labels.astype(np.float64)
    
    print(f"加载完成: {feature_matrix.shape[0]} 行, {len(feature_names)} 个特征")
...
class FeatureAnalyzer:
    ...
    def analyze(self):
        ...
        try:
            ...
            beta_bin, rss_bin, t_values_bin, F_value_bin = ols_regression(y_binary, X)
            ...
            with open(os.path.join(self.output_paths['binary_regression'], f"{self.feature_name}_summary.txt"), "w") as f:
                ...
                f.write(f"Coefficients: {beta_bin}\n")
                f.write(f"Residual Sum of Squares: {rss_bin}\n")
                f.write(f"t-values: {t_values_bin}\n")
                f.write(f"F-value: {F_value_bin}\n")
...
def process_features_from_h5(feature_h5_path, label_h5_path, output_base_path, factor_name_path=None):
    """处理特征分析"""
    feature_matrix, labels, feature_names = load_h5_data(
...
# 结果收集函数
def collect_results(regression_path, output_path, filename):
    """收集回归结果"""
...
def main():
    parser = argparse.ArgumentParser(description="H5特征分析 - 增强版")
    ...
    parser.add_argument("--output_base_path", type=str, 
                       default="/Users/wook/Downloads/FactorTest1/feature_analysis_all_labels_enhanced")
    ...
    try:
        ...
        if os.path.exists(regression_path):
            collect_results(regression_path, args.output_base_path, 'regression_results_all_labels.csv')
        
        binary_regression_path = os.path.join(args.output_base_path, "binaryregression")
        if os.path.exists(binary_regression_path):
            collect_results(binary_regression_path, args.output_base_path, 'binary_regression_results.csv')
